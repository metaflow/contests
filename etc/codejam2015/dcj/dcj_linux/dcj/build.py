...
class Build(object):
  ...
  def AddToParser(self, parser):
    ...
    parser.add_argument('--extra_flags',
                        help='additional flags to pass to compiler. For '
                        'example --extra_flags="-Wall".')
    return parser
  ...
  def _BuildJavaObjectFileCommand(self, c_source, output):
    dcj_root = path.join(path.dirname(path.realpath(__file__)), '..')
    return (
        'gcc', '-c', '-fpic', '-I/usr/lib/jvm/java-7-openjdk-amd64/include',
        '-I/usr/lib/jvm/java/java-7-openjdk-amd64/include/linux',
        '-I' + path.join(dcj_root, 'includes'),
        path.join(dcj_root, 'libraries', c_source),
        '-o', path.join(dcj_root, 'libraries', output),
    )
  ...
  def _JavaBuildCommands(self, args):
    ...
    solution_class_dir = path.dirname(args['source'])
    dcj_root = path.join(path.dirname(path.realpath(__file__)), '..')
    ...
    with open(self.ExecutablePath(args), 'w') as f:
      f.write(
          '#!/bin/bash\ncd {0}\n/usr/bin/java -classpath {1} Wrapper\n'.format(
              solution_class_dir,
              classpath))
    chmod(self.ExecutablePath(args), stat.S_IRWXU | stat.S_IROTH | stat.S_IXOTH)
    ...
    link_object_files = (
        ('ld', '-shared',) +
        object_files +
        ('-o', path.join(dcj_root, 'libraries', 'libmessage.so',))
    )
