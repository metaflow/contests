  ...
  while (tcc--) {
    ...
    for (auto &p : mods) {
      ...
      if (b % 2 == 1) {
        if (b == 1) continue;
        vl ff(b), rr(b);
        for (l i = 0; i < b - 1; i += 2) ff[i] = v[i + 1] - v[i];
        F(i, 1, b) ff[i] += ff[i - 1];
        for (l i = 1; i < b; i += 2) rr[i] = v[i + 1] - v[i];
        B(i, 0, b - 1) rr[i] += rr[i + 1];
        l best = -1;
        for (l i = 0; i < b; i+=2) {
          l f = 0;
          if (i > 1) f = ff[i - 2];
          ...
          if (i < b - 1) r = rr[i + 1];
          l q = r + f;
          if (best == - 1 || best > q) best = q;
        }
        o += best;
      } else {
        for (l i = 0; i < b; i += 2) o += v[i + 1] - v[i];
      }
    }
    out << o << lf;
  }
