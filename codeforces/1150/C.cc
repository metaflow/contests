...
void solve(istream &in, ostream &out) {
  ...
  for (l x : v) if (x == 1) a++; else b++;
  vl w;
  w.reserve(n);
  if (b) { w.emplace_back(2); b--; }
  // TODO: template to output vector
  if (a) { w.emplace_back(1); a--; }
  while (b) { w.emplace_back(2); b--; }
  while (a) { w.emplace_back(1); a--; }
  F(i, 0, n) {
    if (i) out << ' ';
    out << w[i];
  }
