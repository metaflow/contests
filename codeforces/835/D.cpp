...
void solve(istream& cin, ostream& cout) {
  ...
  while (cin >> s) {
    l n = s.size();
    vl v(n); F(i, 0, n) v[i] = s[i] - 'a';
    vvi level(n, vi(n, 0));
    F(i, 0, n) {
      F(j, 0, n) {
        l a = i - j;
        l b = i + j;
        if (a < 0 or b > n or s[a] != s[b]) break;
        level[a][b] = 1;
      }
      F(j, 0, n) {
        l a = i - j;
    ...
    F(w, 1, n / 2 + 1) {
      F(p, 0, n) {
        ...
        l c = p;
        l d = p + w - 1;
        if (a >= 0 and d < n and level[a][d] and level[a][b] and level[c][d]) {
          level[a][d] = level[a][b] + 1;
        }
        c++; d++;
        if (a >= 0 and d < n and level[a][d] and level[a][b] and level[c][d]) {
          level[a][d] = level[a][b] + 1;
    ...
    vl counts(n + 1);
    F(i, 0, n) F(j, 0, n) counts[level[i][j]]++;
    // L(counts, "counts");
    // L(level, "level");
    l total = 0;
    F(i, 1, n + 1) total += counts[i];
    F(i, 1, n + 1) {
      if (i > 1) cout << ' ';
      cout << total;
      total -= counts[i];
