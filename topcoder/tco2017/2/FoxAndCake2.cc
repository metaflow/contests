...
class FoxAndCake2 {
public:
  string isPossible(l c, l s) {
    if (c > s) swap(c, s);
    if (c >= 8 or gcd(c,s) > 1) {
      return "Possible";
    }
    F(i, 2, c) F(j, 2, min((l)100, s)) {
      if (gcd(i, j) == 1) continue;
      if (gcd(c - i, s - j) > 1) return "Possible";
    }
    return "Impossible";
  }
// BEGIN CUT HERE
	...
	void test_case_2() { int Arg0 = 5; int Arg1 = 187576004; string Arg2 = "Possible"; verify_case(2, Arg2, isPossible(Arg0, Arg1)); }
